...
if (!empty($_POST)){
    ...
    if($site2crm_input){
        ...
        $upgrade_message = '<h1><a target="_blank" href="https://www.site2crm.com/pro">'
        ...
        switch(get_option('site2CRM_user_CRM')){
            case 'nutshell': 
                ...
                update_option('Site2CRM_upload_limit', (int)get_option('Site2CRM_upload_limit')+1);
                update_option('Site2CRM_total_leads_created', (int)get_option('Site2CRM_total_leads_created')+1);
                if((int)get_option('Site2CRM_upload_limit')>30){
                    mail($to, 'Site2CRM Lead Creation Limit Reached : no lead created', $upgrade_message . $message, $headers); 
                    break;
                }
